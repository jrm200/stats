...
When we run both programs, we see that the value of the parameters is the same for both methods, as expected. However, when using the marginal density of each $n_i$, the random walk discovers the domain more thoroughly, meaning we have a better chance of finding the true minimum and therefpre the optimal set of parameters $\theta$. 
