...
$96$% confidence interval for the paramater $\beta_1$ have been calculated with the code below.
...
The Weibull distribution adopted previously to model the effects of the added random effects, will be analysed again but this time following a Bayesian estimation procedure. The proposal distributions $q( \boldsymbol{\theta}_j | \boldsymbol{\theta}_{j-1}$ which were used to initiate the Metropolis Hastings method was chosen to be the marginal negative log-likelihood. The starting parameter $\boldsymbol{\theta}_0$ was taken to be the previously computed optimal values for $\theta$ in the early stages of the analysis along with a small parameter estimate for $\log({\sigma}_b)$.
