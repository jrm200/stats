...
The recorded data includes the variables; *litter*, *treatment indicator* (where when rx is 1 indicates the treatment was carried out on the rat and where rx is 0 indicates the rat received the control only), *time* (the follow up time, in weeks, to the appearance of a tumour) and *status* (where status is 1 describes that the presence of a tumour and status is 0 implies there was no tumour).
...
In the first instance, the dependent variable *time*, T_i has been taken to follow a Weibull distribution. 

$$ T_i \sim \mbox{Weibull}(\mbox{shape} = \frac{1}{\sigma}, \mbox{scale}=\exp(\eta_i)), \ \ \ \ \ \ \ \ \eta_i=\beta_0 + \beta_1 x_i $$

Whilst censored observations with rx=0 are described by the survival function of a Weibull distribution given by

$$ S(t|\mbox{a,b})= \exp\bigg(-\bigg( \frac{t}{b}\bigg)^a\bigg), \ \ \ \ \ \ \ \ t> 0 $$ 

The shape parameter in the model takes the form of frac{1}{/sigma} whilst the scale parameter is proportional to exp(\beta_0 + \beta_1 x_i). The data is modelled in this way, so that the basic assumptions of the data are not violated. The maximum likelihood estimate calculates an estimation of the population parameters such that the likelihood of obtaining the observed data is maximised. The vector of parameters that are to be estimated can be expressed as _\theta_^T=( \beta_0, \beta_1, log(\sigma)).
...
A function to describe the likelihood of the parameters _\theta_ is defined as below.

_*insert R code for nll.weibull function*_

In the code it can be seen that *K* is taken to be the shaping parameter of the Weibull distribution. Given that the third element in _\theta_ is proportional to log(\sigma), the exponent of \sigma has been taken in order to neutralise this.
...
Following on from this, the Weibull distribution was again considered, but with added random effect _b_ taken into consideration. So we can now see the scale of the Weibull distribution is then altered to the exponent of \eta_i. Where \eta_i is now as follows:
